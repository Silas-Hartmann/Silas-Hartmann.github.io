...
document.addEventListener('DOMContentLoaded', function() {
  ...
  quizQuestions.forEach((h3, index) => {
    ...
    // Neue Methode zur Überprüfung, ob es eine Aufgabe mit [TYP] Notation ist
...
// NEUE FUNKTION: Prüft, ob es sich um eine Quizfrage im neuen Format handelt
...
// ALTE FUNKTION: Prüft, ob es sich um eine Quizfrage im alten Format handelt
...
// NEUE FUNKTION: Sammelt die Elemente einer Frage bis zum Trennstrich oder zur nächsten Überschrift
...
function showCorrectMCAnswers(question, correctIndices) {
  ...
  if ((questionType === 'multiple-choice' || questionType === 'single-choice') && options.length > 0) {
    ...
    // Bei Multiple-Choice können mehrere Antworten richtig sein
    let correctIndices = [];
    if (correctIndex !== -1) {
    ...
    const gapTextWithDropzones = gapText.replace(/\[([^\]]*)\]/g, (match) => {
      ...
      dropzone.addEventListener('drop', function(e) {
        ...
        // Optional: Entferne das Wort aus der Wortliste
        // Aktuell wird das Wort nicht entfernt, damit man es mehrfach verwenden kann
      });
...
}.insertBefore(itemContainer, container.children[newIndex + 1]);
        } else {
          container.insertBefore(itemContainer, container.children[newIndex]);
        }
